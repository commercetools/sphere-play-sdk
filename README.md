...
We gained a lot of experiences and learnings with this SDK for our SPHERE.IO platform.
To be transparent, let's list some drawbacks
- maintainability: The testability wasn't proper ensured.
- faster development: Integration new endpoints required a lot of code changes.
- independency: The preselection of the Play framework and other libraries restrict it to a smaller user group.

The team put a lot of effort to address those and the result can be found in our new [JVM SDK](https://github.com/sphereio/sphere-jvm-sdk).
...
````scala
libraryDependencies += "io.sphere" %% "sphere-play-sdk" % "0.71.0" withSources()
````
